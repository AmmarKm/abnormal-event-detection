...
//			b_soc = (soc_count >= 2*cb_m);									//overlapping
		...
		if( b_soc ){
			...
			double sx, sy;
			int cc=0;
			double *cb_hist = new double[trDim];        //2014.11.20改為動態  trDim = 64
...
//				Series1->AddArray(cb_hist, 64);
				...
				for(int i=1;i<trDim;i++){     		//search max
					if(cb_hist[i] > cb_max)
						cb_max = cb_hist[i];
					if(cb_hist[i] < cb_min)
...
//				}
				...
				//write data
				wfs.write((unsigned char*)cb_hist, sizeof(cb_hist[0])*trDim);	//需改為sizeof(cb_hist[0])*64

				//記錄data	//stack
				String s = "";
				s = s + cb_hist[0];
				for(int i=1;i<trDim;i++){
					s = s + " " + cb_hist[i];
				}
				dataList->Add(s);
...
//					word_count = Edit11->Text.ToInt();
					...
					Memo1->Lines->Add("host size(part1) = "+WideString(hosf_size));
...
//					String BoF_Filename = initDir + newDir + dataName + dataFileString + "_BoF.bin";
				...
				/* training step 3 */
					...
					wfs2.write((unsigned char*)BoF_DES, sizeof(BoF_DES[0])*dataSize);
